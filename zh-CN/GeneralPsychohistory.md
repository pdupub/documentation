...
不过，现实中确实存在可被系统作为时间的存在，即可以被证实的时间，比如Bitcoin的主链，假如我发送的一条消息中，附带了某个块的签名，必然能够证明这条消息存在于这个块之后。只证明在其之后发生就可以，因为对于时间成本，我们也只要能够证明其附带足够多的时间成本。至于以Bitcoin为代表的的区块链为何可以某种意义上替代时间，或者说替代经典物理中的时间，需要在讨论时间的本质之后，在进行说明。
